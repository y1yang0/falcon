...
func (lir *LIR) lowerArithmetic(val *ssa.Value) {
	...
	case ssa.OpDiv, ssa.OpMod:
		isFloat := val.Type.IsFloat() || val.Type.IsDouble()
		if !isFloat {
			...
			// source operand by the low single precision floating-point value in
			// the second source operand, and stores the single precision
			// floating-point result in the destination operand.
			left := lir.NewVReg(val.Args[0])
			right := lir.NewVReg(val.Args[1])
			result := lir.NewVReg(val)
			lir.NewInstr(LIR_Mov, result, left, result).comment(val)
			lir.NewInstr(LIR_Div, result, right, result).comment(val)
			lir.Bind(val, result)
		}
	case ssa.OpNegate:
		left := lir.NewVReg(val.Args[0])
...
func (lir *LIR) lowerIndexed(val *ssa.Value) {
	...
	case ssa.OpLoadIndex:
		if argVar.Type.IsString() {
			...
			dataRes := freeRegs[0]
			lir.NewInstr(LIR_Mov, dataRes, dataAddr, dataRes).comment(val)
			// load element from dataAddr
			result := lir.NewVReg(val)
			index := lir.NewVReg(argIndex)
			charAddr := lir.NewAddr(LIRTypeByte, dataRes, index, lir.NewOffset(0))
			lir.NewInstr(LIR_Mov, result, charAddr, result).comment(val)
			lir.Bind(val, result)
		} else {
